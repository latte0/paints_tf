...
parser.add_argument('--L1_scale', dest='L1_scale', type=float, default=1.1, help='weight on L1 term in objective')
parser.add_argument('--wasserstein', dest='wasserstein', type=bool, default=False, help='use improved wasserstein discriminator')
parser.add_argument('--selu', dest='SELU', type=bool, default=False, help='use self normalizing activation function')
...
def main(_):
    ...
    with tf.Session() as sess:
        ...
        model = pix2pixmodel(sess,
                        image_size=args.fine_size,batch_size=args.batch_size,output_size=args.fine_size,
                        SELU=args.SELU)

        paintstf = pix2pix(sess, model,
        ...
        paintstf.build_model(model, params)

        if args.phase == 'train':
            paintstf.train(params)
        else:
            paintstf.test(params)
